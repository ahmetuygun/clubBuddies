...
import com.club.buddies.app.config.KafkaProperties;
...
class ClubBuddiesKafkaResourceIT {
    ...
    void setup() {
        ...
        ClubBuddiesKafkaResource kafkaResource = new ClubBuddiesKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/club-buddies-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/club-buddies-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
